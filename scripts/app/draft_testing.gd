...
func _update_draft_recommendations() -> void:
	# TODO: Make weights configurable via UI in the future
	# Currently using default weights: base=0.50, synergy=0.25, counter=0.25
	...
	var next_turn: String = DRAFT_SEQUENCE[_draft_step_index] if _draft_step_index < DRAFT_SEQUENCE.size() else ""
	...
	for champion_id in all_champions:
		if champion_id not in _player_picks and champion_id not in _enemy_picks and champion_id not in _banned_heroes:
	...
	# Get recommendations from native backend with breakdowns
	var recommendations: Array = _backend.get_draft_recommendations_with_breakdowns(
		allies,
		enemies,
		available,
		5,  # top_n
	...
	# Display recommendations with breakdowns
	for i in range(recommendations.size()):
		var breakdown: Dictionary = recommendations[i]
		var champion_id: StringName = breakdown.get("champion", "")
		var base: float = breakdown.get("base", 0.0)
		var synergy: float = breakdown.get("synergy", 0.0)
		var counter: float = breakdown.get("counter", 0.0)
		var final: float = breakdown.get("final", 0.0)

		var label := Label.new()
		label.text = "%d. %s (base: %.3f, synergy: %.3f, counter: %.3f, final: %.3f)" % [i + 1, String(champion_id), base, synergy, counter, final]
		label.add_theme_color_override("font_color", COLOR_TEXT)
