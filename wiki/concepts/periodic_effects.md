...
DoT/HoT effects tick at intervals (default 1.0s tick_interval) for duration. Damage/heal per tick based on: attack_damage_ratio, max_hp_ratio, flat_amount. Stacking modes (string): refresh (reset duration), extend (add duration), stack_damage (add damage per tick), separate (independent instances). StackBehavior enum (for stat modifiers): Refresh, Accumulate, Reset.
