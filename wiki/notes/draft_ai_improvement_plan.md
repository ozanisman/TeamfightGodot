...
|---|---|---|---|
| **Production recommender** | `sim::draft_ai` | `sim_draft_ai_recommender.{hpp,cpp}`, `sim_draft_ai_evaluator.{hpp,cpp}`, `sim_draft_ai_stats_database.{hpp,cpp}`, `sim_draft_ai_types.hpp` | Actual pick/ban recommendations used by gameplay and strategies |
...
- Bridge: `scripts/simulation/native_simulation_backend.gd`
  - Pick: `get_draft_ai_pick_recommendations(stats_dir, available, allies, enemies, max_results, draft_step, strategy)`
  - Ban: `get_draft_ai_ban_recommendations(..., acting_side, weight_overrides, strategy)`
- Strategy wrappers: `scripts/tools/draft_strategy*.gd` (native, random, base_power_only, certified, native_ablation, lookahead variants)
- Gameplay opponent: `scripts/app/simulation_viewer_base.gd` (`_try_enemy_draft_ai`)

### 2.3 Data pipeline

Production stats live in `res://model_stats/stats_output_100k/`, generated by `--generate-stats` (`generate_simulation_stats.gd`) from simulated matches. Files consumed:

- `combat_stats.csv` — per-champion base winrates
- `matchup_with.csv` — ally synergy pairs
- `matchup_vs.csv` — enemy counter pairs
- `role_combinations.csv` — full 5-role composition winrates (with partial-multiset aggregation at query time)

Stats are **static**: loaded once, never updated at runtime, never fed back from live matches.
