...
**C.2 Learned scorer to replace/augment the linear sum.** Validation-only first experiment implemented via `native_draft_state_scorer_experiment.gd`. It trains a compact standardized logistic model on D.1b selected-action draft-state rows, compares against a calibrated native `total_score` baseline, and writes report/model/prediction artifacts. The current validation gate includes calibration bins, grouped held-out diagnostics, and repeated split summaries. `native_learned_scorer` is now available as a validation-only harness strategy that ranks full native candidate pools with the saved scorer model, but it is non-promotable until strength, calibration, side-bias, and realism gates clear. Runtime/native/UI wiring remains gated; no production evaluator export or policy integration belongs in this step.
