...
| 2 | High | `native/src/teamfight_simulation_core.hpp:359-363`, `native/src/teamfight_simulation_core.cpp:1492-1494`, `2393-2395`, `3256-3258` | Broad-phase policy is drifted. Comments and docs say standard 5v5 stays brute, but the code uses `SPATIAL_BROAD_PHASE_TEAM_THRESHOLD = 4`, which means 5v5 can enter the spatial path. That is both a perf question and a behavior question. | Do not change it yet. First decide whether the intended threshold is 4 or 6, then benchmark the exact 5v5 gate before touching runtime behavior. |
...
- Decide whether the intended broad-phase threshold is 4 or 6, then benchmark the exact 5v5 gate again.
- If the threshold stays as-is, focus on reducing per-candidate scoring overhead first.
- If the threshold changes, re-run the same benchmark gate before making any other optimization decisions.
