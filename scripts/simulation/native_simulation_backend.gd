...
func predict_draft_winner(team1: Array, team2: Array, stats_dir: String = "res://stats_output", base_weight: float = 0.50, synergy_weight: float = 0.25, counter_weight: float = 0.25, matchup_weight: float = 0.25, composition_weight: float = 0.0, logistic_k: float = 10.0, include_breakdown: bool = false, synergy_amplification: float = 1.2, matchup_amplification: float = 1.2, score_sharpness: float = 1.0, interaction_weight: float = 0.0, scoring_mode: int = 3, variance_weight: float = 0.0, cc_weight: float = 0.0, mobility_weight: float = 0.0, sustain_weight: float = 0.0, best_counter_weight: float = 0.0, worst_counter_weight: float = 0.0, best_synergy_weight: float = 0.0, worst_synergy_weight: float = 0.0, synergy_aggregation: int = 0, counter_aggregation: int = 0, use_decorrelated_scoring: bool = false, draft_position: int = 0, early_pick_base_weight: float = 0.7, late_pick_counter_weight: float = 0.4) -> Dictionary:
	if not _ensure_native_backend():
		push_error("Simulation backend is not available.")
		return {"team1_prob": 0.5, "team2_prob": 0.5}
	if _backend.has_method("predict_draft_winner"):
		return _backend.call("predict_draft_winner", team1, team2, stats_dir, base_weight, synergy_weight, counter_weight, matchup_weight, composition_weight, logistic_k, include_breakdown, synergy_amplification, matchup_amplification, score_sharpness, interaction_weight, scoring_mode, variance_weight, cc_weight, mobility_weight, sustain_weight, best_counter_weight, worst_counter_weight, best_synergy_weight, worst_synergy_weight, synergy_aggregation, counter_aggregation, use_decorrelated_scoring, draft_position, early_pick_base_weight, late_pick_counter_weight)
