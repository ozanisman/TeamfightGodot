...
Armor and ward reduce incoming damage by percentage. Values are integer points: 22 armor means 22% damage reduction.

Damage types: physical (reduced by armor), magic (reduced by ward), true (ignores all defense). Defense multiplier formula: `1.0 - defense_stat / 100.0` for armor/ward. Negative defense values increase damage taken (defense multiplier > 1.0); there is no upper clamp on the multiplier.
