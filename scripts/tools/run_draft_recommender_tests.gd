...
func _initialize() -> void:
	...
	print("\n========================")
	print("DRAFT-AWARE MODEL VERIFICATION")
	print("========================")

	# Test draft-aware model (scoring_mode=4)
	var draft_aware_aggregate = {
		"score_range": 0.0,
		"mean_margin": 0.0,
		"median_margin": 0.0,
		"rank_volatility": 0.0,
	...
	for i in tests.size():
		...
		var report = core.run_stress_test_with_perturbations(
			...
			1.2,
			1.2,
			4  # DRAFT_AWARE_PAIRWISE_PROBABILITY mode
		)
		...
		draft_aware_aggregate["score_range"] += range_val
		draft_aware_aggregate["mean_margin"] += mean_margin
		draft_aware_aggregate["median_margin"] += median_margin
		draft_aware_aggregate["rank_volatility"] += rank_vol
		draft_aware_aggregate["top3_stability"] += top3_stab
		draft_aware_aggregate["context_sensitivity"] += context_sens

	# Average across scenarios
	draft_aware_aggregate["score_range"] /= tests.size()
	draft_aware_aggregate["mean_margin"] /= tests.size()
	draft_aware_aggregate["median_margin"] /= tests.size()
	draft_aware_aggregate["rank_volatility"] /= tests.size()
	draft_aware_aggregate["top3_stability"] /= tests.size()
	draft_aware_aggregate["context_sensitivity"] /= tests.size()

	print("\nDRAFT-AWARE Model (aggregated across scenarios):")
	print("  Score Range: %.6f" % draft_aware_aggregate["score_range"])
	print("  Mean Margin: %.6f" % draft_aware_aggregate["mean_margin"])
	print("  Median Margin: %.6f" % draft_aware_aggregate["median_margin"])
	print("  Rank Volatility: %.4f" % draft_aware_aggregate["rank_volatility"])
	print("  Top-3 Stability: %.2f%%" % draft_aware_aggregate["top3_stability"])
	print("  Context Sensitivity: %.6f" % draft_aware_aggregate["context_sensitivity"])

	print("\n========================")
	print("RECOMMENDATION")
	print("========================")
	print("  Draft-aware model is the default for all draft prediction tasks.")
	print("  Use DRAFT_AWARE_PAIRWISE_PROBABILITY (scoring_mode=4) for pick recommendations.")
	print("  Use CERTIFIED_PAIRWISE_PROBABILITY (scoring_mode=3) for complete-draft prediction.")
