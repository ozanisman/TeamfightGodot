...
Dictionary build_tick_snapshot(const TickSnapshotInput &input) {
	...
	for (size_t i = 0; i < units.size(); ++i) {
		...
		d["shield"] = u.shield;
		d["mana"] = u.mana;
		d["mana_cost"] = u.stats_dirty ? get_effective_mana_cost(u) : u.cached_mana_cost;
		...
		d["attack_range"] = u.stats_dirty ? get_effective_attack_range(u) : u.cached_attack_range;
		d["attack_speed"] = u.stats_dirty ? get_effective_attack_speed(u) : u.cached_attack_speed;
		d["ability_cd_max"] = u.stats_dirty ? get_effective_ability_cd(u) : u.cached_ability_cd;
