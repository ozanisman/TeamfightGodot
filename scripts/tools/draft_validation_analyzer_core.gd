class_name DraftValidationAnalyzerCore
extends RefCounted

## In-process draft validation summary + A/B report generation
## (native_draft_validation_analyzer.gd core).
...
const DEFAULT_OUTPUT_PATH: String = "res://model_stats/native_draft_validation_summary.csv"
const DEFAULT_AB_OUTPUT_PATH: String = "res://model_stats/native_draft_validation_ab_report.csv"
const DEFAULT_NATIVE_STRATEGY_NAMES: Array[String] = ["native_full", "native_softmax"]


static func run(params: Dictionary) -> Dictionary:
	var input_path: String = String(params.get("input_path", ""))
	var draft_summary_path: String = String(params.get("draft_summary_path", ""))
	var output_path: String = String(params.get("output_path", DEFAULT_OUTPUT_PATH))
	var ab_output_path: String = String(params.get("ab_output_path", DEFAULT_AB_OUTPUT_PATH))
	var native_strategy_names: Array[String] = _coerce_strategy_names(
		params.get("native_strategy_names", DEFAULT_NATIVE_STRATEGY_NAMES)
	)
	var ab_control_blue: String = String(params.get("ab_control_blue", ""))
	var ab_control_red: String = String(params.get("ab_control_red", ""))
	var ab_treatment_blue: String = String(params.get("ab_treatment_blue", ""))
	var ab_treatment_red: String = String(params.get("ab_treatment_red", ""))
	var ab_mde: float = _coerce_float(params.get("ab_mde", 0.05), 0.05, 0.001, INF)
	var ab_alpha: float = _coerce_float(params.get("ab_alpha", 0.05), 0.05, 0.001, 0.5)
	var ab_power: float = _coerce_float(params.get("ab_power", 0.80), 0.80, 0.5, 0.999)

	var drafts: Array[Dictionary]
	if not draft_summary_path.is_empty():
		drafts = DraftValidationCsvScript.load_draft_summaries(draft_summary_path)
	elif not input_path.is_empty():
		drafts = DraftValidationCsvScript.infer_drafts_from_steps(input_path)
	else:
		return _error_result("input_path or draft_summary_path required", output_path, ab_output_path)

	if drafts.is_empty():
		return _error_result("no draft data found", output_path, ab_output_path)
	...
	rows.append_array(_native_by_opponent_rows(drafts, native_strategy_names))
	rows.append_array(_native_by_side_rows(drafts, native_strategy_names))
	...
	if f == null:
		return _error_result("could not open output %s" % output_path, output_path, ab_output_path)
	f.store_string("\n".join(rows) + "\n")
	f.close()

	var ab_rows: Array[String] = _build_ab_report(
		drafts,
		ab_control_blue,
		ab_control_red,
		ab_treatment_blue,
	...
	if not ab_rows.is_empty():
		var ab_f := FileAccess.open(ProjectSettings.globalize_path(ab_output_path), FileAccess.WRITE)
		if ab_f == null:
			return _error_result("could not open A/B output %s" % ab_output_path, output_path, ab_output_path)
		ab_f.store_string("\n".join(ab_rows) + "\n")
		ab_f.close()
	...
	return {
		...
		"ab_output_path": ab_output_path,
		"summary_row_count": rows.size() - 1,
	}


static func _error_result(error: String, output_path: String, ab_output_path: String) -> Dictionary:
	return {
		"ok": false,
		"error": error,
		"output_path": output_path,
...
static func _coerce_strategy_names(value: Variant) -> Array[String]:
	if value is Array:
		...
		if not out.is_empty():
			return out
		return DEFAULT_NATIVE_STRATEGY_NAMES.duplicate()
	if value is String:
		var parsed: Array[String] = _parse_strategy_names(value)
...
static func _parse_strategy_names(s: String) -> Array[String]:
	var out: Array[String] = []
	for part in s.split(","):
...
static func _coerce_float(value: Variant, default_value: float, min_val: float, max_val: float) -> float:
	if value is String and not value.is_valid_float():
		return clampf(default_value, min_val, max_val)
...
static func _csv_field(s: String) -> String:
	if s.find(",") >= 0 or s.find("\"") >= 0 or s.find("\n") >= 0 or s.find("\r") >= 0:
		return "\"" + s.replace("\"", "\"\"") + "\""
	return s


static func _format_metric_row(metric: String, grouping: String, trials: int, wins: int, losses: int, draws: int) -> String:
...
static func _matchup_rows(drafts: Array[Dictionary]) -> Array[String]:
...
static func _native_by_opponent_rows(drafts: Array[Dictionary], native_strategy_names: Array[String]) -> Array[String]:
	var groups: Dictionary = {}
	for draft in drafts:
		var native_info: Dictionary = _native_side(draft, native_strategy_names)
...
static func _native_by_side_rows(drafts: Array[Dictionary], native_strategy_names: Array[String]) -> Array[String]:
	var groups: Dictionary = {}
	for draft in drafts:
		var native_info: Dictionary = _native_side(draft, native_strategy_names)
		if native_info.is_empty():
...
static func _side_overall_rows(drafts: Array[Dictionary]) -> Array[String]:
	var blue_wins: int = 0
	var red_wins: int = 0
	var draws: int = 0
...
static func _native_side(draft: Dictionary, native_strategy_names: Array[String]) -> Dictionary:
	var blue_is_native: bool = native_strategy_names.has(draft["blue_strategy"])
	var red_is_native: bool = native_strategy_names.has(draft["red_strategy"])
...
static func _build_ab_report(
	drafts: Array[Dictionary],
	ab_control_blue: String,
	ab_control_red: String,
	ab_treatment_blue: String,
	ab_treatment_red: String,
	ab_mde: float,
	ab_alpha: float,
	ab_power: float
) -> Array[String]:
	...
	if ab_control_blue.is_empty() or ab_control_red.is_empty():
		return rows

	var control_key: String = "blue=%s red=%s" % [ab_control_blue, ab_control_red]
	if not groups.has(control_key):
		push_warning("DraftValidationAnalyzerCore: control matchup not found: %s" % control_key)
		return rows
	...
	if ab_treatment_blue.is_empty() or ab_treatment_red.is_empty():
		return rows

	var treatment_key: String = "blue=%s red=%s" % [ab_treatment_blue, ab_treatment_red]
	if not groups.has(treatment_key):
		push_warning("DraftValidationAnalyzerCore: treatment matchup not found: %s" % treatment_key)
		return rows
	...
	if control_n <= 0 or treatment_n <= 0:
		push_warning("DraftValidationAnalyzerCore: A/B comparison requires decisive matches in both control and treatment")
		return rows
	...
	var required_n: int = _required_n_per_group(control_p, ab_mde, ab_alpha, ab_power)
	...
	print("A/B: %s (%.1f%% [%.1f%%-%.1f%%]) vs %s (%.1f%% [%.1f%%-%.1f%%]) => delta %.1fpp (95%% CI %.1fpp-%.1fpp), z=%.2f, p=%.4f, required n/group=%d (MDE=%.1f%%)" % [
		...
		z, p_value, required_n, ab_mde * 100.0
	])

	return rows


static func _wilson_ci(wins: int, losses: int, z: float) -> Array:
...
static func _two_proportion_z_test(x1: int, n1: int, x2: int, n2: int) -> Dictionary:
...
static func _required_n_per_group(p0: float, delta: float, alpha: float, power: float) -> int:
...
static func _normal_cdf_inv(p: float) -> float:
