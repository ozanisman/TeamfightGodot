...
**C.2 Learned scorer to replace/augment the linear sum.** Validation-only experiments are implemented for selected-action outcome scoring (`native_draft_state_scorer_experiment.gd`) and candidate-wide selected-action ranking (`native_draft_state_ranker_experiment.gd`). The selected-action policy strategy (`native_learned_scorer`) remains non-promotable after trailing `native_full` and `native_softmax` in ladder validation. The global candidate-wide ranker is currently `VALIDATION_ONLY`: top-1 selected-candidate agreement improved slightly, but MRR and mean selected rank regressed. Grouped/segment diagnostics (`--segment-probes=true`) are implemented and should be used to diagnose the failure mode before any more policy ladder work; the latest native-softmax validation did not identify a promotable pick, ban, or phase-specific segment. Learned-ranker work is paused until a new feature hypothesis is justified by validation data. Runtime/native/UI wiring remains gated until strength, calibration, side-bias, and realism gates clear.
...
Implemented. `native_draft_self_play_stats.gd` accepts optional `--decision-output=...` and writes one CSV row per draft decision with pre-action picks/bans, legal candidate pool, selected action, chosen-candidate diagnostic scores, final draft, and simulated match outcome. It also accepts optional `--candidate-decision-output=...` and writes one row per legal candidate per draft decision with native rank, selected flag, candidate diagnostics, and the same final outcome fields. `native_draft_decision_rows_gate.gd` and `native_draft_candidate_rows_gate.gd` validate the selected-action and candidate-wide row contracts. These are training artifacts only and are not included in `stats_manifest.json`.
...
1. **[E/A] Multi-seed policy candidate selection.** Run the validation-only policy candidate sweep across lookahead/persona candidates with strength, side-bias, A/B, Elo, and realism reports before choosing any promotion target.
